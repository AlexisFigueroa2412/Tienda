...
try {
    ...
    if(isset($_POST['id'])){
        ...
        if ($cliente->readCode($_POST['id'])) {
            if ($rowcliente = $cliente->readEmail($_POST['id'])) {
                $mail->addAddress($rowcliente['correo_cliente'],'Cliente');     //Add a recipient
                //Content
                   $mail->isHTML(true);                                  //Set email format to HTML
                $mail->Subject = utf8_decode('Código de autentificación');
                $mail->Body    = 'Acá te envíamos tu código para que puedas verificar tu identidad <br><b>'.$rowcliente['codigo'].'</b><br> No lo compartas con nadie';
            } else {
